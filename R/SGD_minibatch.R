...
for (epoch in 1:epochs) {
  ...
  for (start in seq(1, n, minibatch_size)) {
    end <- min(start + minibatch_size - 1, n)
    minibatch <- data[start:end, ]
    
    # Extract minibatch variables
    x_minibatch <- minibatch$x
    y_minibatch <- minibatch$y
    
    # Predict the response
    y_pred <- beta_0_hat + beta_1_hat * x_minibatch
    
    # Calculate gradients
    gradient_0 <- -(2 / length(y_minibatch)) * sum(y_minibatch - y_pred)
    gradient_1 <- -(2 / length(y_minibatch)) * sum((y_minibatch - y_pred) * x_minibatch)
